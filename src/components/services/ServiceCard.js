...
function ServiceCard({data,index}){
    return(
        ...
        className="w-full relative p-8 h-80 lg:h-[200pm] border border-gray-300 bg-white hover:shadow-card hover:-translate-y-1 transition duration-300 ease-in-out">
            ...
            <div className="absolute bottom-0 left-0 p-8">
                <Link to={data.href} id={`learn`+index} className="items-end text-xl font-semibold text-gray-900 pt-8">Ver un ejemplo</Link>
            </div>
        </div>
    )
}
